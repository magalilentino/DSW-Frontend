...
export default function ActualizarFormula() {
  const { idFormula } = useParams();
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:3000/api/formula/${idFormula}`)
      .then((res) => res.json())
      .then((data) => {
        const f = data.data;
        setCantidad(f.cantidad);
        setTonoId(f.tono?.idTono || 1);
        setProductoIds(f.productos.map((p: Producto) => p.idProducto));
      });
  }, [idFormula]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) throw new Error("Error al actualizar fórmula");
      navigate("/formula");
    } catch (err) {
